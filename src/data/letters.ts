...
export type MoodGroup = "all" | "emotional" | "daily" | "special" | "support";
...
export const moodFilters: MoodFilter[] = [
  { id: "all", name: "All Moods", icon: "🌟", description: "All letters" },
  { id: "emotional", name: "Emotional Support", icon: "💝", description: "When you need comfort" },
  { id: "daily", name: "Daily Life", icon: "☀️", description: "Everyday moments" },
  { id: "special", name: "Special Occasions", icon: "🎉", description: "Birthdays and celebrations" },
  { id: "support", name: "Tough Times", icon: "🫂", description: "When life gets hard" }
];

// Enhanced categories with better mood groupings
...
// Helper function to get letters by mood group
export const getLettersByGroup = (group: MoodGroup): LetterCategory[] => {
  if (group === "all") {
    return Object.values(letterCategories);
  }
  return Object.values(letterCategories).filter(category => category.group === group);
};

// Helper function to get all letters as a flat array
export const getAllLetters = (): Letter[] => {
  return Object.values(letterCategories).flatMap(category => category.letters);
};
